...
test.describe('help admin support workspace regressions', () => {
  test('support staff can work a selected request across reply, triage, history, and completed queue views', async ({ page }) => {
    const adminCredentials = await ensureSavedAdminCredentials();
    const requestContext = await createAuthenticatedRequestContext(adminCredentials);

    try {
      const fixture = await createSupportCaptureFixture(requestContext, Date.now());
