...
async function createDetachedLibraryForCurrentUser(requestContext, suffix) {
  ...
  const script = [
    ...
    "const detachedSpaceName='Playwright Detached Support Space '+suffix;",
    "const detachedSpaceSlug='playwright-detached-support-space-'+suffix;",
    'const detachedSpace=await pool.query(`INSERT INTO spaces (name, slug, created_by, is_personal) VALUES ($1, $2, $3, false) RETURNING id, name, slug`, [detachedSpaceName, detachedSpaceSlug, userId]);',
    'const detachedSpaceId=Number(detachedSpace.rows[0]?.id||0)||null;',
    "const detachedLibraryName='Detached Support Library '+suffix;",
    'const detachedLibrary=await pool.query(`INSERT INTO libraries (name, created_by, space_id) VALUES ($1, $2, $3) RETURNING id, name, space_id`, [detachedLibraryName, userId, detachedSpaceId]);',
