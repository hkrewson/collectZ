# Pre-2.9.6 Go/No-Go Preflight

- Version: `2.9.6`
- Date: `2026-04-04`
- Commit: local working tree
- Scope: `2.9.6 — Product Edition Boundary and Homelab Surface Definition`
...
- Version metadata is synchronized to `2.9.6` across:
...
- Release note [`docs/releases/v2.9.6.md`](/Users/hamlin/Development/GitHub/hkrewson/collectZ/docs/releases/v2.9.6.md) exists and includes the required release and security-triage sections.
- The in-app release snapshot was regenerated with `node backend/scripts/export-release-feed.js`, and the latest release in [`backend/release-feed.json`](/Users/hamlin/Development/GitHub/hkrewson/collectZ/backend/release-feed.json) is `2.9.6`.
- Running stack rebuilt with `APP_VERSION=2.9.6`, `APP_EDITION=platform`, and `PLAYWRIGHT_E2E_BYPASS_TOKEN=collectz-playwright`; live `/api/health` reports `version=2.9.6`, `frontend=2.9.6`, `backend=2.9.6`.
...
- Playwright browser regression is green locally at `17 passed`, `4 skipped`.
- RBAC regression passed against the running stack when executed with `BASE_URL=http://frontend:3000`.
- Homelab edition-boundary smoke passed against a live `APP_EDITION=homelab` backend container.
