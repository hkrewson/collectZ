...
function MetricButton({ label, value, onClick, disabled = false }) {
  ...
  if (!onClick) {
    return <div className="rounded-lg border border-edge bg-panel px-3 py-2.5">{content}</div>;
  }
  return (
    <button
      type="button"
      ...
      className="rounded-lg border border-edge bg-panel px-3 py-2.5 text-left transition hover:border-dim hover:bg-raised/40 disabled:cursor-default disabled:hover:border-edge disabled:hover:bg-panel"
    >
      {content}
    </button>
  );
    ...
    <div className="h-full overflow-y-auto p-4 sm:p-5">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ghost">{scopeLabel}</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.72fr)]">
        <Panel title="Needs attention">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-edge" role="tablist" aria-label="Needs attention sections">
              {attentionTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`border-b-2 px-0 pb-2 text-sm transition ${
                      active
            ...
            <div role="tabpanel" aria-label={activeAttention.label}>
            ...
            <div className="space-y-2">
              {recentActivity.slice(0, DASHBOARD_SAMPLE_LIMIT).map((item) => (
                <div key={item.id} className="border-t border-edge/70 pt-2 first:border-t-0 first:pt-0">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="truncate text-sm font-medium text-ink">{item.title || item.action}</p>
                    <span className="text-xs text-ghost">{formatDateTime(item.created_at)}</span>
                  </div>
                  <p className="mt-1 truncate text-xs text-ghost">{item.action}{item.summary ? ` · ${item.summary}` : ''}</p>
                </div>
              ))}
            </div>
          ) : (
