...
process.env.INTEGRATION_ENCRYPTION_KEY = process.env.INTEGRATION_ENCRYPTION_KEY || 'plex-watch-state-refresh-smoke-key';
const { encryptSecret } = require('../services/crypto');
const { ensureUserDefaultScope } = require('../services/libraries');

const BASE_URL = process.env.BASE_URL || 'http://localhost:3000';
const fakePlexToken = `plex-watch-refresh-${crypto.randomBytes(6).toString('hex')}`;
const ARTIFACT_PATH = path.resolve(
  ...
  'artifacts',
  'plex-watch-state',
  'plex-watch-state-refresh-scheduler-smoke.json'
);
...
async function startFakePmsServer() {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/library/metadata/3001') {
      res.writeHead(200);
      res.end(JSON.stringify({
        MediaContainer: {
          Metadata: [{
            ratingKey: '3001',
            type: 'movie',
            title: 'Watch Refresh Movie',
            viewCount: 1,
            lastViewedAt: 1778260000,
            duration: 7200000,
            viewOffset: 0,
            Media: [{ Part: [{ file: '/mnt/plex-media/Watch Refresh Movie.mkv' }] }]
          }]
        }
      }));
      return;
    }
    if (req.method === 'GET' && url.pathname === '/library/metadata/4001/allLeaves') {
      res.writeHead(200);
      res.end(JSON.stringify({
        MediaContainer: {
          Metadata: [
            {
              ratingKey: '4001-1',
              grandparentRatingKey: '4001',
              type: 'episode',
              grandparentTitle: 'Watch Refresh Show',
...
async function main() {
  ...
  const email = `plex-watch-refresh-${suffix}@example.com`;
  const password = `Passw0rd!${crypto.randomBytes(4).toString('hex')}`;
  const client = new HttpClient('plex-watch-refresh');
  let userId = null;
  const mediaIds = [];
  try {
    userId = await createDirectUser({ email, password, name: 'Plex Watch Refresh Admin' });
    ...
    const status = await client.request('/api/media/plex-watch-state/refresh-scheduler', { expectStatus: 200 });
    assert(status.data?.processingMode === 'scheduled_watch_state_refresh', `Unexpected scheduler status: ${JSON.stringify(status.data)}`);
    assert(status.data?.runtime?.enabled === false, 'Expected watched-state refresh timer to default off');
    ...
    const movie = await pool.query(
      `INSERT INTO media (title, media_type, format, library_id, space_id, added_by, import_source)
       VALUES ('Watch Refresh Movie', 'movie', 'Digital', $1, $2, $3, 'manual')
       RETURNING id`,
    ...
    await pool.query(
      `INSERT INTO media_metadata (media_id, "key", "value")
       VALUES ($1, 'plex_item_key', '1:3001')`,
      [movieId]
    );

    const show = await pool.query(
      `INSERT INTO media (title, media_type, format, library_id, space_id, added_by, import_source)
       VALUES ('Watch Refresh Show', 'tv_series', 'Digital', $1, $2, $3, 'manual')
       RETURNING id`,
      [libraryId, spaceId, userId]
    );
    ...
    const refresh = await client.request('/api/media/plex-watch-state/refresh-scheduler/run', {
      method: 'POST',
      withCsrf: true,
      expectStatus: 200,
      body: { maxItems: 2 }
    ...
    assert(refresh.data?.processingMode === 'scheduled_watch_state_refresh', `Unexpected refresh mode: ${JSON.stringify(refresh.data)}`);
    assert(refresh.data?.plexWriteback === false, 'Expected Plex writeback to stay disabled');
    assert(refresh.data?.summary?.scopeCount === 1, `Expected one scoped refresh group: ${JSON.stringify(refresh.data?.summary)}`);
    assert(refresh.data?.summary?.scopesProcessed === 1, `Expected one processed scope: ${JSON.stringify(refresh.data?.summary)}`);
    assert(refresh.data?.summary?.mediaMatched === 2, `Expected two matched media rows: ${JSON.stringify(refresh.data?.summary)}`);
    assert(refresh.data?.summary?.mediaMetadataUpdated === 1, `Expected one movie metadata update: ${JSON.stringify(refresh.data?.summary)}`);
    assert(refresh.data?.summary?.seasonsUpdated === 1, `Expected one season update: ${JSON.stringify(refresh.data?.summary)}`);
    ...
    const evidence = {
      ok: true,
      provider: 'plex',
      processingMode: refresh.data.processingMode,
      schedulerDefaultEnabled: status.data.runtime.enabled,
      ...
      assertions: [
        'Plex watched-state refresh scheduler defaults the timer off',
        'Manual scheduler sweep reused the watched-state apply path',
        'No new media rows were created during watched-state refresh',
        'Plex scrobble and unscrobble writeback paths were not called'
      ]
    };
    assertSecretFree(evidence, 'watch-state refresh evidence');
