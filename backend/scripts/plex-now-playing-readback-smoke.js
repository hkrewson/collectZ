...
const fakeToken = `plex-now-playing-readback-${crypto.randomBytes(8).toString('hex')}`;
...
async function main() {
  ...
  const email = `plex-now-playing-${suffix}@example.com`;
  const password = `Passw0rd!${crypto.randomBytes(6).toString('hex')}`;
  const client = new HttpClient('plex-now-playing-readback-smoke');
  ...
  try {
    ...
    userId = await createDirectUser({ email, password, name: 'Plex Now Playing Smoke Admin', role: 'admin' });
    ...
    const response = await client.request('/api/admin/settings/integrations/test-plex-now-playing', {
      method: 'POST',
      withCsrf: true,
      expectStatus: 200,
      body: JSON.stringify({}),
    ...
    assert(response.data?.ok === true, `Expected successful now-playing probe: ${JSON.stringify(response.data)}`);
    assert(response.data?.path === '/status/sessions', `Expected now-playing path readback: ${JSON.stringify(response.data)}`);
    assert(response.data?.sessionCount === 1, `Expected one session: ${JSON.stringify(response.data)}`);
    const session = response.data?.sessions?.[0];
    assert(session?.title === 'Safe Readback', `Expected sanitized session title: ${JSON.stringify(session)}`);
    assert(session?.progressPercent === 25, `Expected progress readback: ${JSON.stringify(session)}`);
    assert(session?.player?.state === 'playing', `Expected player state: ${JSON.stringify(session)}`);
    ...
    assert(!JSON.stringify(response.data).includes('/private/media'), 'Response must not contain media file paths');
    assert(!JSON.stringify(response.data).includes('192.168.1.24'), 'Response must not contain player IP addresses');
    ...
    console.log(JSON.stringify({
      ...
      sessionTitles: response.data.sessions.map((entry) => entry.title),
