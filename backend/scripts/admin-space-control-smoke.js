...
async function main() {
  ...
  const inviteResponse = await admin.request('/api/admin/invites', {
    ...
    withCsrf: true,
    expectStatus: 201,
    body: { email: ownerEmail, expose_token: true }
  });
  const inviteToken = inviteResponse?.data?.token;
  assert(Boolean(inviteToken), 'Invite token missing for new owner bootstrap');

  await owner.fetchCsrfToken();
  const registerOwner = await owner.request('/api/auth/register', {
    method: 'POST',
    withCsrf: true,
    expectStatus: 200,
    body: { email: ownerEmail, password: ownerPassword, name: 'Phase5 Owner', inviteToken }
  });
  ...
  const spaceId = Number(createSpace?.data?.id || 0);
