...
async function main() {
  ...
  const email = `import-normalization-${suffix}@example.com`;
  const password = 'Passw0rd!123';
  const client = new HttpClient('normalization-smoke');
  let userId = null;
  let libraryId = null;
  let spaceId = null;
  ...
  try {
    userId = await createDirectUser({
      email,
      password,
      name: 'Import Normalization Smoke User'
    });
    ...
    const seeded = await pool.query(
      `INSERT INTO media (
       ...
       RETURNING id`,
      [
        'Alpha Flight #10: Blood Battle! / Family Ties',
        JSON.stringify({
          series: 'Alpha Flight',
          issue_number: '10',
          volume: '1'
    ...
    const seededMediaId = Number(seeded.rows[0]?.id || 0) || null;
    if (!seededMediaId) throw new Error('Failed to seed existing comic row');

    const csv = [
      'title,media_type,format,series,issue_number,volume',
      '"Alpha Flight #10","comic_book","Digital","Alpha Flight","10","1"'
    ].join('\n');

    const form = new FormData();
    form.append('file', new Blob([csv], { type: 'text/csv' }), 'normalization-smoke.csv');
    ...
    if (Number(summary.created || 0) !== 0) {
      throw new Error(`Expected no created rows, got summary=${JSON.stringify(summary)}`);
    }
    if (Number(summary.updated || 0) !== 1) {
      throw new Error(`Expected one updated row, got summary=${JSON.stringify(summary)}`);
    }
    if (firstAuditRow?.match_mode !== 'matched_by_normalization_high') {
      throw new Error(`Expected matched_by_normalization_high, got audit=${JSON.stringify(firstAuditRow)}`);
    }
    if (firstAuditRow?.matched_by !== 'normalization_series_issue_volume') {
      throw new Error(`Expected normalization_series_issue_volume, got audit=${JSON.stringify(firstAuditRow)}`);
    }
    ...
    if (count !== 1) {
      throw new Error(`Expected one comic row in scoped library after import, found ${count}`);
    }

    console.log('Import normalization smoke passed');
