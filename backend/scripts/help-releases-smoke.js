...
const EXPECTED_VERSION = process.env.EXPECTED_RELEASE_VERSION || require('../package.json').version;
...
async function main() {
  ...
  const email = `help-releases-smoke-${suffix}@example.com`;
  const password = `Passw0rd!${crypto.randomBytes(6).toString('hex')}`;
  const client = new HttpClient('help-releases-smoke');
  let userId = null;
  let libraryId = null;
  let spaceId = null;
  ...
  try {
    userId = await createDirectUser({
      email,
      password,
      name: 'Help Releases Smoke Admin',
      role: 'admin'
    });
    ...
    const releasesResponse = await client.request('/api/support/releases?limit=5', { expectStatus: 200 });
    const releases = Array.isArray(releasesResponse?.data?.releases) ? releasesResponse.data.releases : [];
    const versions = releases.map((entry) => String(entry?.version || '')).filter(Boolean);
    assert(versions.length > 0, `Expected Help > Releases feed entries, got ${JSON.stringify(releasesResponse?.data)}`);
    assert(versions.includes(EXPECTED_VERSION), `Expected Help > Releases to include ${EXPECTED_VERSION}, got ${JSON.stringify(versions)}`);

    console.log(JSON.stringify({
      expectedVersion: EXPECTED_VERSION,
      latestVersion: versions[0] || null,
      releaseCount: releases.length,
      foundExpectedVersion: versions.includes(EXPECTED_VERSION),
