...
process.env.INTEGRATION_ENCRYPTION_KEY = process.env.INTEGRATION_ENCRYPTION_KEY || 'plex-rating-apply-smoke-key';
const { encryptSecret } = require('../services/crypto');
const { ensureUserDefaultScope } = require('../services/libraries');

const BASE_URL = process.env.BASE_URL || 'http://localhost:3000';
const fakePlexToken = `plex-rating-apply-${crypto.randomBytes(6).toString('hex')}`;
const ARTIFACT_PATH = path.resolve(__dirname, '..', '..', 'artifacts', 'plex-ratings', 'plex-rating-apply-smoke.json');
...
async function startFakePmsServer() {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/library/metadata/5101') {
      res.writeHead(200);
      res.end(JSON.stringify({
        MediaContainer: {
          Metadata: [{
            ratingKey: '5101',
            type: 'movie',
            title: 'Rating Apply Movie',
            userRating: 8.5,
            rating: 6.1,
            Media: [{ Part: [{ file: '/mnt/plex-media/Rating Apply Movie.mkv' }] }]
          }]
        }
      }));
      return;
...
async function main() {
  ...
  const email = `plex-rating-apply-${suffix}@example.com`;
  const password = `Passw0rd!${crypto.randomBytes(4).toString('hex')}`;
  const client = new HttpClient('plex-rating-apply');
  let userId = null;
  const mediaIds = [];
  try {
    userId = await createDirectUser({ email, password, name: 'Plex Rating Apply Admin' });
    ...
    const movie = await pool.query(
      `INSERT INTO media (title, media_type, format, library_id, space_id, added_by, import_source, user_rating)
       VALUES ('Rating Apply Movie', 'movie', 'Digital', $1, $2, $3, 'manual', 2)
       RETURNING id`,
    ...
    await pool.query(
      `INSERT INTO media_metadata (media_id, "key", "value")
       VALUES ($1, 'plex_item_key', '1:5101')`,
      [movieId]
    );

    const beforeCount = await pool.query('SELECT COUNT(*)::int AS count FROM media WHERE library_id = $1', [libraryId]);
    const apply = await client.request('/api/media/apply-plex-ratings', {
      method: 'POST',
      withCsrf: true,
      expectStatus: 200,
      body: { ratingKeys: ['5101'] }
    });

    assert(apply.data?.processingMode === 'rating_apply', `Unexpected processing mode: ${JSON.stringify(apply.data)}`);
    assert(apply.data?.plexWriteback === false, 'Expected Plex rating writeback to stay disabled');
    assert(apply.data?.summary?.mediaMatched === 1, `Expected one matched media row: ${JSON.stringify(apply.data?.summary)}`);
    assert(apply.data?.summary?.ratingsUpdated === 1, `Expected one rating update: ${JSON.stringify(apply.data?.summary)}`);

    const updated = await pool.query(
      `SELECT user_rating FROM media WHERE id = $1`,
      [movieId]
    );
    assert(Number(updated.rows[0]?.user_rating) === 8.5, `Expected user_rating 8.5: ${JSON.stringify(updated.rows[0])}`);

    const metadata = await pool.query(
      `SELECT "key", "value" FROM media_metadata WHERE media_id = $1 AND "key" LIKE 'plex_rating_%' OR (media_id = $1 AND "key" = 'plex_user_rating') ORDER BY "key"`,
      [movieId]
    );
    const metadataMap = new Map(metadata.rows.map((row) => [row.key, row.value]));
    assert(metadataMap.get('plex_user_rating') === '8.5', `Expected plex_user_rating metadata: ${JSON.stringify(metadata.rows)}`);
    ...
    assert(!fake.requests.some((entry) => entry.pathname === '/:/rate'), 'Smoke must not call Plex rating writeback path');
    ...
    const evidence = {
      ...
      metadataRating: metadataMap.get('plex_user_rating'),
      ...
      assertions: [
        'Plex rating readback updated existing collectZ user_rating',
        'No new media rows were created during rating apply',
        'Plex /:/rate writeback path was not called'
      ]
    };
    assertSecretFree(evidence, 'rating apply evidence');
