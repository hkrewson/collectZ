...
- platform bridge API routing, `VITE_PLATFORM_API_URL` runtime wiring, and stale Core browser shell coverage for moved platform workspace/user administration routes
...
7. Move global workspace/member administration to `cairn`, backed by documented Core APIs where Core data changes are required. In progress: `cairn` now owns the workspace directory and user-route API contract; collectZ Core returns 404 for the global `/api/admin/spaces*` control-plane, no longer documents those paths, and no longer carries the old unreachable handlers, admin-space control smoke, or stale workspace-manager browser shell spec tied to the moved routes. Core still owns user/workspace-scoped `/api/spaces*` operations and workspace integrations.
