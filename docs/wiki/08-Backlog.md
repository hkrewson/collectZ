...
## NEXT To Do After Comic-Con Readiness

### Backlog Item: Maintainability Guardrails and Frontend Extraction Foundation
**Type:** release-candidate engineering foundation
**Candidate milestone:** `3.x.0 — Maintainability Guardrails and Frontend Extraction Foundation`
**Tags:** `maintainability`, `frontend`, `linting`, `testing`, `accessibility`, `modularity`, `ci`
**Status:** NEXT To Do after Comic-Con readiness is pushed, CI-verified, and field-test stabilization has had time to surface urgent trip blockers.
...
- Recent codebase review identified real structural debt in large files such as `frontend/src/components/EventsView.jsx`, `frontend/src/components/LibraryView.jsx`, `backend/routes/media.js`, and `backend/routes/events.js`.
- The current release discipline is strong, but style/quality control is mostly enforced through manual review, custom tests, Playwright regression, Docker checks, and release gates rather than lint/format tooling.
- Playwright coverage is meaningful and should remain the regression layer; the gap is fast, isolated frontend tests for extracted helpers/components.
- Accessibility work exists, but it is inconsistent and not lint-enforced across frontend files.
- This work should start only after the Comic-Con `3.22.0` readiness build is safely pushed and tested enough to avoid destabilizing the travel release candidate.

**Scope**
- Add ESLint and Prettier in a non-disruptive mode first; avoid mass-formatting unrelated files.
- Add React hooks linting and `eslint-plugin-jsx-a11y` so accessibility consistency improves as files are touched.
- Add Vitest for frontend utilities/components first; keep the existing backend custom harness and smoke scripts intact.
- Extract one small real slice from `EventsView.jsx` or `LibraryView.jsx` as the proving ground for the strangler pattern.
- Add touched-file size reporting or a warning-only size ratchet for the largest source files before making any hard CI failure rule.
...
- Do not migrate the backend unit harness to Vitest.
- Do not introduce Redux or broad app-wide state management.
- Do not mass-format the repo.
- Do not code-split top-level routes until lint/test guardrails are stable.
- Do not perform a big-bang rewrite of `media.js`, `events.js`, `EventsView.jsx`, or `LibraryView.jsx`.

**Candidate subtasks**
- Add lint/format dependencies and scripts at the root/frontend level with warn/report behavior.
- Add CI/local-gate reporting for lint output without blocking existing release flows on day one.
- Create the first frontend Vitest setup with one or two extracted pure helpers from an active UI surface.
- Pick the first extraction target from real upcoming product work rather than synthetic cleanup.
- Add documentation for extraction expectations and file-size ratchet behavior.
- Decide when lint warnings become blocking after the baseline is visible and manageable.

**Acceptance Criteria**
- Running the local quality command reports lint/format/a11y findings without requiring unrelated mass edits.
- The first extracted frontend slice has focused unit coverage and existing Playwright regression still passes.
...
- The roadmap entry for the promoted milestone clearly states what is warning-only versus blocking.
...
**Candidate subtasks**
- Promote the current builder functions in `drawerMetadata.js` into registry entries. Completed for the current shared metadata set.
