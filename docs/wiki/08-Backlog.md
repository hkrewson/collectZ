...
### Backlog Item: Dependency PR and CI Security Coverage Review
**Type:** Maintenance review
**Tags:** `security`, `ci`, `dependencies`, `github`, `vulnerability-management`
**Status:** Active backlog; prompted by the current open GitHub PR queue and at least one Snyk vulnerability PR.

**Goal:** Review the current dependency/security PR queue and decide whether collectZ should expand CI vulnerability and supply-chain testing.

**Why this work exists**
- GitHub currently has a visible backlog of open PRs, including dependency/security automation and one Snyk PR indicating a vulnerability.
- The project already has CI gates for dependency scanning, secret scanning, image security/SBOM, compose smoke, browser regression, RBAC regression, and edition boundaries.
- We need a deliberate review before merging, closing, or expanding security automation so the project does not drift into either stale vulnerable dependencies or noisy unactionable security checks.

**Intent**
- Turn the open PR queue into a clear security/dependency maintenance decision instead of reactive one-off merges.
- Confirm whether the existing CI workflow catches the right classes of vulnerability for a homelab container app.
- Identify useful improvements without adding noisy checks that produce more maintenance burden than risk reduction.

**Current state**
- There are multiple open GitHub PRs to triage, including at least one Snyk vulnerability PR.
- Dependency and container security gates exist, but their coverage, severity thresholds, schedules, artifacts, and failure policies need a focused review.
- The backlog does not yet have a single task that owns dependency PR triage plus CI security coverage review.

**Scope**
- Inventory the open PRs by source, affected package/image/workflow, severity, risk, and merge readiness.
- Prioritize any vulnerability-bearing PRs, especially Snyk findings.
- Review whether current CI gates are enough for dependency, image, secret, and supply-chain risk.
- Propose concrete CI workflow improvements only after the PR/vulnerability review establishes the actual gaps.

**Candidate subtasks**
- List the open GitHub PRs and classify them as security fix, dependency update, CI/workflow change, app change, stale, duplicate, or unknown.
- For each security/dependency PR, record affected package or image, current version, target version, vulnerability identifier when available, severity, exploitability notes, and whether collectZ runtime paths appear exposed.
- Decide the disposition for each open PR: merge, rebase, replace with a local update, close, defer, or investigate further.
- Review the existing CI gates and document what each one proves: `dependency-scan`, `secret-scan`, `image-security-and-sbom`, `compose-smoke`, `rbac-regression`, `browser-regression`, `homelab-edition-boundary`, and `platform-edition-boundary`.
- Evaluate whether to add or adjust security coverage such as CodeQL, OSV/Scorecard, Snyk workflow policy, npm audit severity thresholds, lockfile drift checks, license scanning, SBOM diffing, container base-image cadence, provenance/attestation checks, scheduled weekly security scans, and automated dependency grouping.
- Define a false-positive and emergency-patch policy so future security findings have a known path from alert to release.
- Document which checks should block PRs, which should warn, and which should run on a scheduled cadence.

**Out of scope**
- Do not merge or close dependency PRs merely because this backlog item exists.
- Do not add new CI vendors or paid/security-service dependencies until the promoted milestone chooses that direction.
- Do not change release policy, stable/latest tagging, or vulnerability disclosure policy in the same slice unless explicitly promoted.
- Do not expose secret-bearing CI artifacts, logs, or scanner output while gathering evidence.

**Acceptance Criteria**
- Each open GitHub PR has a documented disposition and rationale.
- The Snyk vulnerability PR has severity, affected component, runtime relevance, and a fix or deferral plan documented.
- Current CI security gates have a clear coverage map and known gaps.
- Any recommended CI additions are written as follow-up tasks or promoted milestones, not hidden in ad hoc workflow churn.
- The review distinguishes verified facts from inference and marks any blocked GitHub/CI evidence explicitly.
...
### Backlog Item: Collection Health Rules and Identifier Expectations
**Type:** Deferred minor candidate
**Tags:** `product`, `health`, `review`, `identifiers`, `metadata`, `classification`
**Status:** Active backlog; current missing-identifier Review rules are intentionally simple and need a richer product model before expanding.

**Goal:** Replace the current broad "missing identifiers" rule with media-type, format, source, and provider-aware collection health rules.

**Why this work exists**
- Dashboard Review now explains why missing-identifier rows are flagged, but the underlying rule is still blunt: rows without UPC, ISBN/Google Books, TMDB, Plex, or Kavita identity are treated as missing identifiers.
- That is useful for books, movies, TV, scanner imports, Plex/Kavita rows, and many physical retail items.
- It is less helpful for digital/manual items, especially audio, where a UPC may not exist or may not be the right improvement path.
- The app needs to distinguish "this item needs a stable identifier" from "this item has sparse descriptive metadata."

**Intent**
- Make Review and future Collection Health feel helpful instead of scolding users about impossible cleanup.
- Preserve identifier-first accuracy where identifiers are genuinely expected.
- Add a separate vocabulary for metadata completeness so digital/manual/local records can be improved without pretending every item should have a retail barcode.

**Current state**
- The missing-identifier backend filter flags rows only when every recognized identity field is absent.
- Review row clues can recommend identifier types, such as ISBN/Google Books for books or TMDB/Plex for movies and TV.
- Audio rows currently fall back to UPC recommendations, even when the row may represent a digital item, local file, or non-retail release.
- There is no persisted health finding model, no per-media-type expectation policy, and no distinction between identifier gaps and sparse metadata gaps.

**Proposed model**
- Split collection-health findings into at least two families:
  - `missing_identifier`: a stable external identity is expected or strongly useful for this row.
  - `sparse_metadata`: the row is usable but missing descriptive fields that would improve search, matching, display, or future enrichment.
- Evaluate expectations from media type, owned format, import source/provider, and existing fields.
- Keep the Review labels user-facing and plain; avoid exposing internal rule names except in debug/readback.

**Candidate rule direction**
- Books:
  - Prefer ISBN/ISBN-13 or Google Books identity.
  - Flag missing identifiers when a book lacks ISBN/Google Books and was not intentionally entered as a loose/manual note-only item.
- Movies and TV:
  - Prefer TMDB identity, Plex identity, or another strong provider identity.
  - Keep missing-identifier review for unlinked manual titles that can reasonably be matched.
- Comics:
  - Prefer UPC/ISBN, provider issue identity, Kavita chapter/series identity, or enough series/issue/volume metadata to support matching.
  - Treat "series + issue" as metadata health even when no retail barcode exists.
- Audio:
  - Do not flag digital/manual audio solely for missing UPC.
  - For physical retail audio, UPC/EAN can remain an identifier recommendation.
  - For digital/manual audio, classify missing artist, album/title, year, source/provider, or format as sparse metadata instead.
- Games:
  - Prefer UPC/EAN for physical retail games where format/source suggests a packaged item.
  - For digital/manual games, prefer platform, developer/publisher, release year, and source/provider metadata before demanding UPC.
- Art/collectibles:
  - Keep these out of generic missing-identifier rules unless a later milestone defines domain-specific identifiers.
  - Prefer sparse metadata findings such as missing artist, medium, edition/run, maker, series, vendor, or event linkage.

**Candidate subtasks**
- Inventory current `missing_identifiers` counts by media type, format, and import source in a real dev dataset.
- Define a versioned health-rule contract for each media type: expected identifiers, helpful metadata, and suppressing conditions.
- Add a shared backend health-classification service that returns finding type, reason, recommendation, severity, and source fields.
- Replace the current hardcoded missing-identifier SQL with rule-aware backend classification or a staged query-plus-classifier approach.
- Update Dashboard Review and Library review filters to separate `IDs` from future `Metadata` or `Sparse data`.
- Add explainable row clues for each finding family.
- Add tests for digital audio, physical audio, books, movies/TV, comics, games, and provider-linked rows.
- Update OpenAPI and release docs when promoted.

**Out of scope**
- Do not add automatic enrichment, provider writeback, or broad data repair in the same slice.
- Do not require UPC for all audio or digital items.
- Do not introduce a vague collection score until the underlying finding rules are explainable.
- Do not migrate this into Universal Search; this is a health/review classification model.
...
- The promoted milestone includes runtime proof against representative real rows, not only source inspection.
