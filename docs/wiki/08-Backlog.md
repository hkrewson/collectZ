...
Promoted as `3.4.91`; the read-only progress sync contract follow-up was promoted as `3.4.100`, the first read-only progress visibility implementation was promoted as `3.4.101`, the first explicit progress writeback/page proxy slice was promoted as `3.4.102`, the mark read/unread contract was promoted as `3.4.103`, and the chapter mark-read implementation was promoted as `3.4.104`. Keep series-wide mark read/unread, volume-wide mark read, chapter unread, full iframe reader ownership, PDF/raw chapter file proxying, background polling, KOReader sync, and shared provider abstractions as separate backlog tasks until selected.
