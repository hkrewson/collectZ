...
### Backlog Item: Loan Reminder Workflow
**Type:** Task
**Tags:** `loans`, `reminders`, `email`, `workflow`, `library`
...
**Why this work exists**
- `3.3.0` establishes the core loans workflow, but reminder delivery adds a second layer of behavior:
  - eligibility timing,
  - send actions,
  - duplicate-send guardrails,
  - delivery status,
  - and overdue follow-up handling.
- Reminder behavior should build on a proven loans record lifecycle instead of being bundled into the first loans foundation slice by default.
...
- The reminder design is ready for later automation without requiring a rewrite of the loan record shape.
