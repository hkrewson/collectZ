...
49. `Kavita Comic Series Title Normalization and Issue Mapping` was promoted as `3.4.153`; `Kavita Comic Issue Coverage Guardrails` was promoted as `3.4.154`; `Kavita Numeric Comic Library Type Fan-out Fix` was promoted as `3.4.155`; keep true chapter unread, embedded reader ownership, PDF/raw chapter file proxying, background polling, KOReader sync, repair of older already-imported rows, cover 404 recovery, issue coverage audit, and shared provider abstractions separate.
